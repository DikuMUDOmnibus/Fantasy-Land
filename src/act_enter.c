...
ROOM_INDEX_DATA  *get_random_room(CHAR_DATA *ch)
{
	...
	&&   !IS_SET(room->room_flags, ROOM_WAR)
...
{    
	...
	if (portal->item_type != ITEM_PORTAL 
        ||  (IS_SET(portal->value[1],EX_CLOSED) && !IS_IMMORTAL( ch ) ) )
	{
	    send_to_char("You can't seem to find a way in.\n\r",ch);
	    return;
	}

	if (!IS_IMMORTAL(ch) && !IS_SET(portal->value[2],GATE_NOCURSE)
	...
	&&   !IS_IMMORTAL(ch) )
	{
	    act("$p leads to an unlinked area.",ch,portal,NULL,TO_CHAR,POS_DEAD);
	    return;
	}
