...
void do_get( CHAR_DATA *ch, char *argument )
{
	...
	switch ( container->item_type )
	{
		...
		return;
            ...
            case ITEM_FURNITURE:
                if ( !IS_SET(container->value[2],SIT_ON)
...
void do_restring( CHAR_DATA *ch, char *argument )
{
    ...
    {
	act("$N says '{SNo freebies!{x'",ch,NULL,stringer,TO_CHAR,POS_RESTING);
...
bool remove_obj( CHAR_DATA *ch, int iWear, bool fReplace, bool show )
{
    ...
    if ( obj->item_type == ITEM_DEMON_STONE )
    for ( pet=char_list; pet!=NULL; pet=pet_next ) { pet_next=pet->next;
    if ( pet->master==ch && IS_NPC(pet) && pet->pIndexData->vnum == MOB_VNUM_DEMON ) {
        stop_follower(pet);
        if (pet->in_room != NULL)
...
bool obj_droptest( CHAR_DATA *ch, OBJ_DATA *obj )
{
	...
	for ( mult = obj->multi_data; mult != NULL; mult = mult->next )
	{
	    ...
	    {
		send_to_char("{RNo Multiplaying {R!{x\n\r",ch->master);
...
void do_repair( CHAR_DATA *ch, char *argument )
{
	...
	if ( number_percent() < 40 + ( chance / 2 ) )
	{
		...
		ch, obj, NULL, TO_CHAR, POS_DEAD );
	    act( "$n attempts to repair $p, but fails.",
		ch, obj, NULL, TO_ROOM, POS_RESTING );
	    check_improve( ch, gsn_repair, FALSE, 5 );
	} else {
	    act( "Your attempt to repair $p etches it more!",
		ch, obj, NULL, TO_CHAR, POS_DEAD );
	    act( "$n attempts to repair $p and etches it more!",
		ch, obj, NULL, TO_ROOM, POS_RESTING );
	    check_improve( ch, gsn_repair, FALSE, 5 );
