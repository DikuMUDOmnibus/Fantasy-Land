...
void do_goto( CHAR_DATA *ch, char *argument )
{
    ...
    CHAR_DATA *pet;
    if ( argument[0] == '\0' ) for ( pet=char_list; pet; pet=pet->next ) if ( pet->master == ch && IS_NPC(pet) ) {
	char_from_room( pet );
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
    ...
    int pets=0;
    CHAR_DATA *pet;
    char pet_name_list[MSL];
    pet_name_list[0]='\0';
    for ( pet=char_list; pet; pet=pet->next ) if ( pet->master == ch ) {
     pets++;
     strcat( pet_name_list, pet->name );
    }
    sprintf( buf, "Master: %s  Leader: %s  Pets: %d\n\r",
	victim->master      ? victim->master->name   : "(none)",
	victim->leader      ? victim->leader->name   : "(none)", pets );
    send_to_char(buf,ch);
    if ( pets > 0 ) {
     send_to_char( "Pet list:\n\r", ch );
     send_to_char( pet_name_list, ch );
    }
...
void do_string( CHAR_DATA *ch, char *argument )
{
	...
	victim->zone = NULL;
		...
		return;
    	    ...
    	    victim->pcdata->who_descr = str_dup( end_string(arg3,16) );
    	    return;
    	}
...
void do_preturn( CHAR_DATA *ch, char *argument )
{
	...
	char_to_room( ch, ch->pcdata->was_in_room );
        CHAR_DATA *pet;
        for ( pet=char_list; pet; pet=pet->next ) if ( IS_NPC(pet) && pet->master == ch )
	{
	    char_from_room( pet );
...
void do_plist( CHAR_DATA *ch, char *argument )
{
	...
	int err=system( "dir -1 ../player/*/* >../player/names.txt" );
        if ( err < 0 ) log_string( "Error: do_plist system call\n\r");
     }
	...
	{
	    if ( initial == '\0' ) {
		int err=system( "dir -1 ../backup/*/* >../player/names.txt" );
        if ( err < 0 ) log_string( "Error: backup dir -1 system call\n\r");
            }
	    else
	    {
...
void do_rlist( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' )
    {
	send_to_char( "Syntax:  rlist < area / name >\n\r", ch );
	return;
    }

    fAll  = !str_cmp( argument, "area" );
    final = new_buf( );
    bool found=FALSE;
    if ( fAll ) {
	...
	&&   ( fAll || !str_infix_2( argument, strip_color( pRoomIndex->name ) ) ) )
	{
            found=TRUE;
	    sprintf( buf, "[%5d] %s ",
	...
	add_buf ( final, "\n" );
     ...
     for ( ; hash<MAX_KEY_HASH; hash++ ) {
      for ( pRoomIndex=room_index_hash[hash]; pRoomIndex; pRoomIndex=pRoomIndex->next ) {
       if ( !str_infix_2( argument, strip_color( pRoomIndex->name ) ) ) {
            found=TRUE;
            AREA_DATA *pArea = find_area( pRoomIndex->vnum );
            char *temp=str_dup( end_string( pArea?pArea->name:"", 13 ) );
	    sprintf( buf, "[%5d] %s %s\n",
		pRoomIndex->vnum, end_string( pRoomIndex->name, 50 ), temp );
	    add_buf( final, buf );
            free_string(temp);
       }
      }
    ...
    if ( found != TRUE )
	send_to_char( "No rooms like that were found.\n\r", ch );
    else
	page_to_char( final->string, ch );
