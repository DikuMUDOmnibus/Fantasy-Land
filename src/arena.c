...
void arena_start( ARENA_DATA *match )
{
	...
	char_to_room( wch, get_room_index(
        ...
        CHAR_DATA *pet;
        for ( pet=char_list; pet; pet=pet->next ) if ( IS_NPC(pet) && pet->master == wch ) {
	    char_from_room( pet );
	    char_to_room( pet, wch->in_room );
	    pet->arena_number = match->number;
	}
...
void check_arena( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	char_to_room( victim, get_room_index( ROOM_VNUM_ALOSER ) );
        CHAR_DATA *pet;
        for ( pet=char_list; pet; pet=pet->next ) if ( IS_NPC(pet) && pet->master == victim ) {
            char_from_room( pet );
