...
void nanny( DESCRIPTOR_DATA *d, char *argument )
{
	...
	case CON_GET_NAME:
	    if ( argument[0] == '\0' )
	    {
		send_to_desc( "{RYou must pick a name!{x\n\r", d );
//		close_socket( d );
		return;
	    }
	...
	case CON_READ_MOTD:
		...
		if ( ch->pcdata->tier == 1 )
		{
			...
			set_title( ch, ", the newest victim." );
		...
		wiznet("$N has connected.",
            ...
            for ( pet=char_list; pet; pet=pet_next ) { pet_next=pet->next;
            if ( IS_NPC(pet) && pet->master == ch )
	    {
                if ( pet->in_room ) char_from_room(pet);
...
#define FNDELAY O_NDELAY
    ...
    write_to_buffer( dnew, "Connected to ", 0 );
    write_to_buffer( dnew, VERSION, 0 );
	...
	send_to_desc( "{cWhich {Ds{wou{Dl {cdares seek {Dab{wan{Wd{wo{Wn{wme{Dnt{c?{x\n\r", dnew );
    }
    return;
}
...
//        case 'M': i = (vch == to) ? "you"  : HIM_HER(vch);  break;
        case 'z': i = (ch == to)  ? "your" : HIS_HER(ch);   break;
        case 'Z': i = (vch == to) ? "your" : HIS_HER(vch);  break;
...
char *colour( char type, CHAR_DATA *vch )
{
	...
	case 'A':	/* Auction Channel */
		...
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_auc, ch));
	    else
		sprintf( clcode, "%s", channel( "barter", C_B_BLUE ) );
	    break;
	case 'E':	/* Clan Gossip Channel */
	    if (ch->pcdata && ch->pcdata->color_cgo)
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_cgo, ch));
	    else
		sprintf( clcode, "%s", channel( "clangossip", C_B_BLUE ) );
	    break;
	case 'F':	/* Clan Talk Channel */
	    if (ch->pcdata && ch->pcdata->color_cla)
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_cla, ch));
	    else
		sprintf( clcode, "%s", C_B_MAGENTA );
	    break;
	case 'H':	/* Gossip Channel */
	    if (ch->pcdata && ch->pcdata->color_gos)
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_gos, ch));
	    else
		sprintf( clcode, "%s", channel( "gossip", C_B_BLUE ) );
	    break;
	case 'J':	/* Grats Channel */
	    if (ch->pcdata && ch->pcdata->color_gra)
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_gra, ch));
	    else
		sprintf( clcode, "%s", channel( "grats", C_YELLOW ) );
	    break;
	case 'K':	/* Group Tell Channel */
	    if (ch->pcdata && ch->pcdata->color_gte)
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_gte, ch));
	    else
		sprintf( clcode, C_CYAN );
	    break;
	case 'L':	/* Immortal Talk Channel */
	    if (ch->pcdata && ch->pcdata->color_imm)
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_imm, ch));
	    else
		sprintf( clcode, "%s", channel( "immtalk", C_B_RED ) );
	    break;
	case 'P':	/* Question+Answer Channel */
	    if (ch->pcdata && ch->pcdata->color_que)
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_que, ch));
	    else
		sprintf( clcode, "%s", channel( "answer", C_B_YELLOW ) );
	    break;
	case 'Q':	/* Quote Channel */
	    if (ch->pcdata && ch->pcdata->color_quo)
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_quo, ch));
	    else
		sprintf( clcode, "%s", channel( "quote", C_GREEN ) );
	    break;
	case 'S':	/* Say Channel */
	    if (ch->pcdata && ch->pcdata->color_say)
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_say, ch));
	    else
		sprintf( clcode, C_MAGENTA );
	    break;
	case 'T':	/* Shout+Yell Channel */
	    if (ch->pcdata && ch->pcdata->color_sho)
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_sho, ch));
	    else
		sprintf( clcode, C_RED );
	    break;
	case 'U':	/* Tell+Reply Channel */
	    if (ch->pcdata && ch->pcdata->color_tel)
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_tel, ch));
	    else
		sprintf( clcode, C_CYAN );
	    break;
	case 'V':	/* Wiznet Messages */
	    if (ch->pcdata && ch->pcdata->color_wiz)
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_wiz, ch));
	    else
		sprintf( clcode, C_WHITE );
	    break;
	case 'a':	/* Mobile Talk */
	    if (ch->pcdata && ch->pcdata->color_mob)
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_mob, ch));
	    else
		sprintf( clcode, C_MAGENTA );
	    break;
	case 'd':	/* Chat Channel */
	    if (ch->pcdata && ch->pcdata->color_cht)
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_cht, ch));
	    else
		sprintf( clcode, C_CYAN );
	    break;
	case 'e':	/* Room Title */
	    if (ch->pcdata && ch->pcdata->color_roo)
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_roo, ch));
	    else
		sprintf( clcode, C_B_BLUE );
	    break;
	case 'f':	/* Opponent Condition */
	    if (ch->pcdata && ch->pcdata->color_con)
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_con, ch));
	    else
		sprintf( clcode, C_B_RED );
	    break;
	case 'h':	/* Fight Actions */
	    if (ch->pcdata && ch->pcdata->color_fig)
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_fig, ch));
	    else
		sprintf( clcode, C_B_BLUE );
	    break;
	case 'i':	/* Opponents Fight Actions */
	    if (ch->pcdata && ch->pcdata->color_opp)
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_opp, ch));
	    else
		sprintf( clcode, C_CYAN );
	    break;
	case 'j':	/* Disarm Messages */
	    if (ch->pcdata && ch->pcdata->color_dis)
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_dis, ch));
	    else
		sprintf( clcode, C_B_YELLOW );
	    break;
	case 'k':	/* Witness Messages */
	    if (ch->pcdata && ch->pcdata->color_wit)
		sprintf( clcode, "%s", colour_channel(ch->pcdata->color_wit, ch));
	    else
		sprintf( clcode, "%s", colour_clear( ch ));
	    break;
	case 'l':	/* Quest Gossip */
            if (ch->pcdata && ch->pcdata->color_qgo)
                sprintf( clcode, "%s", colour_channel(ch->pcdata->color_qgo, ch));
            else
                sprintf( clcode, "%s", channel( "questgossip", C_B_BLUE ) );
            break;
        case 'n':       /* OOC */
            if (ch->pcdata && ch->pcdata->color_ooc)
                sprintf( clcode, "%s", colour_channel(ch->pcdata->color_ooc, ch));
            else
                sprintf( clcode, "%s", channel( "ooc", C_B_CYAN ) );
            break;
        case 'o':       /* RACE */
            if (ch->pcdata && ch->pcdata->color_rac)
                sprintf( clcode, "%s", colour_channel(ch->pcdata->color_rac, ch));
            else
                sprintf( clcode, "%s", channel( "race", C_B_BLUE ) );
            break;
        case 'p':       /* FLAME */
            if (ch->pcdata && ch->pcdata->color_fla)
                sprintf( clcode, "%s", colour_channel(ch->pcdata->color_fla, ch));
            else
                sprintf( clcode, "%s", channel( "flame", C_B_RED ) );
            break;
	case 'q':
            if (ch->pcdata && ch->pcdata->color_olc1)
                sprintf( clcode, "%s", colour_channel(ch->pcdata->color_olc1, ch));
            else
                sprintf( clcode, C_YELLOW);
	    break;
	case 's':
            if (ch->pcdata && ch->pcdata->color_olc2)
                sprintf( clcode, "%s", colour_channel(ch->pcdata->color_olc2, ch));
            else
                sprintf( clcode, C_B_BLUE );
	    break;
	case 't':
            if (ch->pcdata && ch->pcdata->color_olc3)
                sprintf( clcode, "%s", colour_channel(ch->pcdata->color_olc3, ch));
            else
                sprintf( clcode, C_WHITE );
	    break;
        case 'u':       /* HERO */
            if (ch->pcdata && ch->pcdata->color_her)
                sprintf( clcode, "%s", colour_channel(ch->pcdata->color_her, ch));
            else
                sprintf( clcode, "%s", channel( "herotalk", C_GREEN ) );
            break;
        case 'v':       /* IC */
            if (ch->pcdata && ch->pcdata->color_ic)
                sprintf( clcode, "%s", colour_channel(ch->pcdata->color_ic, ch));
            else
                sprintf( clcode, "%s", channel( "ic", C_YELLOW ) );
            break;
        case '9':
            if (ch->pcdata && ch->pcdata->color_pra)
                sprintf( clcode, "%s", colour_channel(ch->pcdata->color_pra, ch));
            else
                sprintf( clcode, C_B_RED);
            break;
	case '{':
...
char *colour2( char type )
{
	...
	default:
	    break;
        case '`': sprintf( clcode, INVERSE ); break;
	case '<':
	    sprintf( clcode, B_BLACK );
         break;
	case '>':
	    sprintf( clcode, "\x1b[0m" ); //ANSI_NORMAL );
         break;
	case '.':
	...
	case ':':
	    sprintf( clcode, B_BLUE );
         break;
	case ',':
	    sprintf( clcode, B_PURPLE );
         break;
	case '\'':
	    sprintf( clcode, B_CYAN );
         break;
	case '"':
	    sprintf( clcode, B_GREY );
         break;
	case 'x':
        case '0':
	    sprintf( clcode, CLEAR );
...
{
 send_to_char( "Disabled by root administrator.\n",ch);
 	return;
...
void copyover_recover ()
{
 return;
    ...
    for ( ; ; )
    {
	int err = fscanf (fp, "%d %d %s %s %s\n", &desc, &compress, name, host, hostip);
        if ( err < 0 ) log_string( "copyover_recover: fscanf could not read expected data" );
...
void do_test( CHAR_DATA *ch, char *argument )
{
	...
	int err=system( "dir -1 ../player/*/* >../player/names.txt" );
        if ( err < 0 ) log_string( "Error: player/names.txt system call\n\r");
	...
	int err=system( "dir -1 ../player/*/* >../player/names.txt" );
        if ( err < 0 ) log_string( "Error: player/names system call\n\r");
