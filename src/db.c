...
#define IMPORTED_FILE_PREFIX "lor-"
#define IMPORT_VNUM_OFFSET 100000
int import_vnum_offset=0;  // a number is added to this, see IMPORT_LIST below
bool isImport=FALSE;
...
/*
 * ALSO KNOWN AS THE NIMUD FILE FORMAT (CREATED BY LOCKE)
 */
void load_area_asgard( FILE *fp, bool importing )
{
    AREA_DATA *pArea;
    char      *word;
    bool      fMatch;
    ...
    if ( !importing )
     ...
     char buf[MSL];
     char *p=strArea;  while ( *p != '\0' && *p != '/' ) p++;  if ( *p == '\0' ) p=strArea; else p++;
     sprintf( buf, "%s%s", IMPORTED_FILE_PREFIX, p );
    ...
    for ( ; ; )
    {
       ...
       switch ( UPPER(word[0]) )
       {
           ...
           case 'V':
            ...
            {
                pArea->min_vnum = import_vnum_offset + fread_number( fp );
                pArea->max_vnum = import_vnum_offset + fread_number( fp );
            }
            break;
           case 'E':
             if ( !str_cmp( word, "End" ) )
...
void load_rooms_asgard( FILE *fp, bool importing )
{
    ROOM_INDEX_DATA *pRoomIndex;
	...
	vnum				= import_vnum_offset + fread_number( fp );
	if ( vnum == 0 )
	    break;
...
void load_asgard_obj( FILE *fp, bool importing )
{ 
    OBJ_INDEX_DATA *pObjIndex;
    char *spell;
	...
	SET_BIT( pObjIndex->extra_flags, ITEM_NO_SAC );
        ...
        case ITEM_PIT: 
            pObjIndex->value[0]         = fread_number(fp); 
            pObjIndex->value[1]         = fread_flag(fp); 
            pObjIndex->value[2]         = import_vnum_offset + fread_number(fp); 
            pObjIndex->value[3]         = fread_number(fp); 
            pObjIndex->value[4]         = fread_number(fp); 
            break; 
        case ITEM_DRINK_CON: 
...
void load_asgard_mob( FILE *fp, bool importing )
{
    MOB_INDEX_DATA *pMobIndex; 
        ...
        { 
            ...
            { 
             ...
             { 
                ...
                SET_BIT( pMobIndex->mprog_flags, trigger ); 
                pMprog->trig_type       = trigger; 
                pMprog->vnum            = import_vnum_offset + fread_number( fp ); 
                pMprog->trig_phrase     = fread_string( fp ); 
                pMprog->next            = pMobIndex->mprogs; 
                pMobIndex->mprogs       = pMprog; 
             } 
...
void load_voting_polls()
{
    ...
    int err= system( "dir -1 ../data/voting_polls >../data/voting_polls/polls.txt" );
        if ( err < 0 ) log_string( "Error: load_voting_polls system call\n\r");
...
void load_area( FILE *fp, bool importing )
{
    AREA_DATA *pArea;
    char      *word;
    bool      fMatch;
...
//    pArea->credits	= fread_string( fp );
    ...
    for ( ; ; )
    {
       ...
       switch ( UPPER( word[0] ) )
       {
	    ...
	    case 'B':
		if ( !importing ) { SKEY( "Bldr",	pArea->builder	); }
		break;
	    ...
	    case 'R':
		KEY( "Room", pArea->run_vnum,	import_vnum_offset + fread_number( fp ) );
		break;
        }
    }
}
...
void load_help( FILE *fp )
{
	...
	switch( UPPER( *word ) )
	{
		...
		LOAD( "Keyw", pHelp->keyword, fread_string( fp ) );
                if ( strlen(pHelp->name) > 40 ) { // fixes some ghastly bug
                 free_string(pHelp->name);
                 pHelp->name=str_dup(pHelp->keyword);
                }
		break;
	    ...
	    case 'I':
		LOAD( "Immtext", pHelp->immtext, fread_string( fp ) );
		break;
	}
    }
}
...
void load_mobiles( FILE *fp, bool importing )
{
    MOB_INDEX_DATA *pMobIndex;
    char *temp;
    sh_int pos;
	...
	if ( ( vnum = import_vnum_offset + fread_number( fp ) ) == 0 )
	    break;
...
void load_objects( FILE *fp, bool importing )
{
    OBJ_INDEX_DATA *pObjIndex;
    char *word;
    sh_int pos;
	...
	switch( pObjIndex->item_type )
	{
	    ...
	    case ITEM_EXIT:
		pObjIndex->value[0]	= import_vnum_offset + fread_number( fp );
		pObjIndex->value[1]	= fread_flag( fp );
		pObjIndex->value[2]	= fread_flag( fp );
		pObjIndex->value[3]	= fread_flag( fp );
		pObjIndex->value[4]	= fread_flag( fp );
	...
	for ( ; ; )
	{
	    ...
	    {
		pObjIndex->forge_vnum	= import_vnum_offset + fread_number( fp );
		...
		SET_BIT( pObjIndex->oprog_flags, trigger );
		pOprog->trig_type	= trigger;
		pOprog->vnum		= import_vnum_offset + fread_number( fp );
		pOprog->trig_phrase	= fread_string( fp );
		pOprog->next		= pObjIndex->oprogs;
		pObjIndex->oprogs	= pOprog;
	    }
...
void load_rooms( FILE *fp, bool importing )
{
    EXIT_DATA *pexit;
    EXTRA_DESCR_DATA *ed;
    PROG_LIST *pRprog;
	...
	if ( load_asgard_area )
	{
	    load_rooms_asgard( fp, importing );
	    return;
	}
    #endif
	...
	for ( ; ; )
	{
		...
		case 'D':
		    ...
		    pexit->key			= import_vnum_offset + fread_number( fp );
		    pexit->u1.vnum		= import_vnum_offset + fread_number( fp );
		    pexit->orig_door		= door;
		    pRoomIndex->exit[door]	= pexit;
		    top_exit++;
		    break;
		...
		case 'R':
		    ...
		    SET_BIT( pRoomIndex->rprog_flags, trigger );
		    pRprog->trig_type	= trigger;
		    pRprog->vnum	= import_vnum_offset + fread_number( fp );
		    pRprog->trig_phrase	= fread_string( fp );
		    pRprog->next	= pRoomIndex->rprogs;
		    pRoomIndex->rprogs	= pRprog;
		    break;
...
void load_resets( FILE *fp, bool importing )
{
    RESET_DATA *pReset;
	...
	pReset->arg1	= isImport && 
                        ...
                        || letter == 'E'
                        || letter == 'R' )
                         ? import_vnum_offset + fread_number(fp)
                         : fread_number( fp );
	pReset->arg2	= fread_number( fp );
	...
	switch ( letter )
	{
		...
		||   !( pexit = pRoomIndex->exit[pReset->arg2] ) )
		{
		    ...
		    if ( isImport == FALSE ) exit( 1 ); else break;
		}
...
void load_mobprogs( FILE *fp, bool importing )
{
    PROG_CODE *pMprog;
...
void load_shops( FILE *fp, bool importing )
{
    SHOP_DATA *pShop;
	...
	pShop->keeper		= import_vnum_offset + fread_number( fp );
	if ( pShop->keeper == 0 )
	    break;
...
void load_specials( FILE *fp, bool importing )
{
    for ( ; ; )
    {
	MOB_INDEX_DATA *pMobIndex;
...
void load_social_table( void )
{
    ...
    if ( err == EOF ) {
     fclose(fp);
     bug( "Error (%d) fscanf in load_social_table\n", err );
     return;
    }
...
void load_objprogs( FILE *fp, bool importing )
{
    PROG_CODE *pOprog;
...
void load_roomprogs( FILE *fp, bool importing )
{
    PROG_CODE *pRprog;
...
void boot_db( )
{
	...
	for ( ; ; )
	{
	    ...
	    if ( word[0] == '$'               )                 break;
	    else if ( !str_cmp( word, "AOD_AREA" ) ) load_area	    (fpArea,FALSE);
//	    else if ( !str_cmp( word, "AREA"	 ) ) load_area_ds   (fpArea);
//	    else if ( !str_cmp( word, "AREADATA" ) ) load_area_ashen(fpArea);

#if defined LOAD_ASGARD_AREAS
	    else if ( !str_cmp( word, "AREADATA" ) ) load_area_asgard(fpArea,FALSE);
#endif

	    else if ( !str_cmp( word, "MOBILES"  ) ) load_mobiles   (fpArea,FALSE);
	    ...
	    else if ( !str_cmp( word, "RESETS"   ) ) load_resets    (fpArea,FALSE);
	    else if ( !str_cmp( word, "ROOMS"    ) ) load_rooms     (fpArea,FALSE);
	    else if ( !str_cmp( word, "SHOPS"    ) ) load_shops     (fpArea,FALSE);
	    else if ( !str_cmp( word, "SPECIALS" ) ) load_specials  (fpArea,FALSE);
	    else
	    {
    ...
    log_string( "Reading Import List." );
    if ( ( fpList = fopen( IMPORT_LIST, "r" ) ) != NULL )
    {
    isImport=TRUE;
    import_vnum_offset=IMPORT_VNUM_OFFSET;
	...
	for ( ; ; )
	{
	    ...
	    if ( word[0] == '$'               )                 break;
	    else if ( !str_cmp( word, "AOD_AREA" ) ) load_area	    (fpArea,TRUE);
//	    else if ( !str_cmp( word, "AREA"	 ) ) load_area_ds   (fpArea);
//	    else if ( !str_cmp( word, "AREADATA" ) ) load_area_ashen(fpArea);

#if defined LOAD_ASGARD_AREAS
	    else if ( !str_cmp( word, "AREADATA" ) ) load_area_asgard(fpArea,TRUE);
#endif

	    else if ( !str_cmp( word, "MOBILES"  ) ) load_mobiles   (fpArea,TRUE);
	    else if ( !str_cmp( word, "MOBPROGS" ) ) load_mobprogs  (fpArea,TRUE);
	    else if ( !str_cmp( word, "OBJPROGS" ) ) load_objprogs  (fpArea,TRUE);
	    else if ( !str_cmp( word, "ROOMPROGS") ) load_roomprogs (fpArea,TRUE);
	    else if ( !str_cmp( word, "OBJECTS"  ) ) load_objects   (fpArea,TRUE);
	    else if ( !str_cmp( word, "RESETS"   ) ) load_resets    (fpArea,TRUE);
	    else if ( !str_cmp( word, "ROOMS"    ) ) load_rooms     (fpArea,TRUE);
	    else if ( !str_cmp( word, "SHOPS"    ) ) load_shops     (fpArea,TRUE);
	    else if ( !str_cmp( word, "SPECIALS" ) ) load_specials  (fpArea,TRUE);
	    else
	    {
		bug( "Boot_db: bad section name.", 0 );
		exit( 1 );
    ...
    fclose( fpList );
    char b[MSL];
    sprintf( b, "rm -f %s", IMPORT_LIST );
    int err = system( b );
    isImport=FALSE;
    log_string( "Done importing." );
    // close out these
    }
...
// from nimud:
       ...
       if ( pRoom->people != NULL
         && !IS_SET(pExit->rs_flags, EX_CONCEALED) )
       {
         if ( IS_SET(pExit->rs_flags, EX_WINDOW) )
         {
          ...
          {
          act( "The $t shuts.", pRoom->people, pExit->keyword, NULL, TO_ACTOR, POS_RESTING );
          act( "The $t shuts.", pRoom->people, pExit->keyword, NULL, TO_SCENE, POS_RESTING );
          }
         }
         else
         {
          if ( !IS_SET(pExit->rs_flags, EX_CLOSED)
            && IS_SET(pExit->rs_flags, EX_CLOSED) )
          {
          act( "The $t closes.", pRoom->people, pExit->keyword, NULL, TO_ACTOR, POS_RESTING );
          act( "The $t closes.", pRoom->people, pExit->keyword, NULL, TO_SCENE, POS_RESTING );
          }
         }
         ...
         {
         act( "The $t clicks.", pRoom->people, pExit->keyword, NULL, TO_ACTOR, POS_RESTING );
         act( "The $t clicks.", pRoom->people, pExit->keyword, NULL, TO_SCENE, POS_RESTING );
         }
       }
...
//		level = URANGE( 0, pMob->level - 2, LEVEL_HERO - 1 );
	    ...
	    case 'O':
                if ( IS_SET(pRoom->room_flags,ROOM_SAVE_CONTENTS) ) {
                 // Stops endless loading of objects in rooms with this flag...
                 break;
                }
...
char *fread_string( FILE *fp ) {
    ...
    static char localcopy[MAX_STRING_LENGTH];
    char c=' ';
   ...
   if ( c == EOF ) {
    bug( "Fread_string: unexpected EOF", 0 );
    return &str_empty[0];
   }
   ...
   if (  c == '~' )
    return &str_empty[0];
         ...
         *   -- Furey
         * Fuck this shit it needs a rewrite. -- Locke
         */
    ...
    *p=c;
    ...
    while ( !feof(fp) ) {
        if ( r > MAX_STRING_LENGTH-3 ) {
         bug( "Fread_string: Majoro issuoso domo arigatou, string too long bitch", 0 );
         break;
        }
        c = getc(fp);
        if ( c == EOF ) {
    ...
    if ( r > MAX_STRING_LENGTH-3 ) {
     bug( "Fread_string: Majoro issuoso domo arigatou, string too long bitch", 0 );
     localcopy[MAX_STRING_LENGTH-3] = '\0';
    }
    if ( !str_cmp( localcopy, "(null)" ) ) return &str_empty[0];
    if ( !str_cmp( localcopy, "(null) " ) ) return &str_empty[0];
    return str_dup( localcopy );
}
 ...
 * From NIMUD
 */
bool str_infix_nim( const char *astr, const char *bstr )
{
    int sstr1;
    int sstr2;
    int ichar;
 ...
 * Written by Locke
 */
bool str_infix_2( const char *astr, const char *bstr ) {
 if ( astr == NULL || *astr == '\0' ) {
  log_string( "Str_infix: null astr (compared against).\n"  );//, bstr );
  return TRUE;
 }
 if ( bstr == NULL || *bstr == '\0' ) {
  log_string( "Str_infix: null bstr (compared against '%s').\n" );//, astr );
  return TRUE;
 }
 char *key;
 char *p,*q;
...
void log_string( const char *str )
{
    ...
    fp = fopen (buf, "a");
    if ( !fp ) {
     printf( "Fail on file open (log_string), file `%s` string was: %s\n", buf, str );
     return;
    }
