...
int xp_compute( CHAR_DATA *gch, CHAR_DATA *victim, int total_levels )
{
    ...
    xp = number_range (
      (int) ((float) xp * 10/4.0f),
      (int) ((float) xp * 20/4.0f)
    );
	...
	xp = (int)( (float) xp * 0.75f );

    // adjust for grouping
    xp = xp * gch->level * 3 / ( UMAX( 1, total_levels -1 ) );

    /* adjust for global exp_mod */
    float f1=(float)mud_stat.exp_mod[0];
    float f2=(float)mud_stat.exp_mod[1]; if ( f2 == 0 ) f2=1;

    xp = (int) ( (f1 * (float) xp) / f2 );
...
void do_flee( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->wait > 0 && number_range(0,5) > 2 ) {
     send_to_char( "You are too out of breath right now.\n\r", ch );
     return;
    }
    ...
    int doors[MAX_DIR];
    int i=0;
    for ( i=0; i<MAX_DIR; i++ ) doors[i]=-1;
    for ( i=0; i<MAX_DIR; i++ ) {
     int candidate=number_range(0,MAX_DIR);
     if ( candidate >= MAX_DIR ) candidate = MAX_DIR-1;
     int attempts=MAX_DIR*MAX_DIR;
     while ( doors[candidate] != -1 && attempts>0 ) {
      candidate=number_range(0,MAX_DIR);
      if ( candidate >= MAX_DIR ) candidate = MAX_DIR-1;
      attempts--;
     }
     doors[candidate]=i;
    }
    for ( i=0; i<MAX_DIR; i++ ) if ( doors[i] == -1 ) doors[i]=number_range(0,MAX_DIR);

    for ( i=0; i<MAX_DIR; i++ ) {
     char buf[MSL];
     sprintf( buf, "debug: %d %d %d %d %d %d\n\r", doors[0], doors[1], doors[2], doors[3], doors[4], doors[5] );
    }
    ...
    {
	int door = doors[attempt];
	EXIT_DATA *pexit = was_in->exit[door];

	if ( pexit == NULL
	||   pexit->u1.to_room == NULL
	|| IS_SET(pexit->exit_info, EX_CLOSED)
        || ( IS_SET(pexit->exit_info, EX_WINDOW) && pexit->key == -1 )
        || (IS_SET(pexit->exit_info, EX_CLOSED && IS_SET(pexit->exit_info,EX_NOPASS) && !IS_AFFECTED(ch, AFF_PASS_DOOR)) )
        || IS_SET(pexit->exit_info, EX_NOMOVE)
	|| ( IS_NPC(ch)
	...
	if ( number_range(0,ch->daze[DAZE_FLEE]) != 0 ) {
         send_to_char( "In your daze you fail to flee ", ch );
         send_to_char( dir_name[door], ch );
         send_to_char( "!\n\r", ch );
         continue;
        }

        if ( number_range(0,ch->mod_stat[STAT_DEX]) > victim->mod_stat[STAT_DEX]/2 
          && number_range(0,victim->mod_stat[STAT_STR]) > ch->mod_stat[STAT_STR]/2 ) {
          send_to_char( "You failed to escape because your opponent blocked you!\n\r", ch);
          return;
        }
    ...
    if ( failed == MAX_DIR ) send_to_char( "You cannot find a way out!\n\r", ch );
    else
...
void death_cry( CHAR_DATA *ch, CHAR_DATA *killer )
{
    ...
    if ( ch->riding != NULL )
    {
        act( "As $n dies, you fall to the ground.",
             ch, NULL, ch->riding, TO_VICT, POS_RESTING );
        ch->riding->position   = POS_RESTING;
    }

    if ( ch->riding != NULL )
    act( "$n falls off you, dead.", ch, NULL, ch->riding, TO_VICT, POS_RESTING );
...
void do_feed( CHAR_DATA *ch, char *argument )
{
    ...
    if ( time_info.month >= 15 
     ...
     && (weather_info.sky == SKY_RAINING || weather_info.sky == SKY_LIGHTNING) ) {
      send_to_char( "You don't feel like it - you're satiated from the blood rain.\n\r", ch );
      return;
    }
