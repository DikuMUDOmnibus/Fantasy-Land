...
bool in_group( CHAR_DATA *ch, CHAR_DATA *victim )
{
...
ROOM_INDEX_DATA *random_room() {
 int start=number_range(0,MAX_KEY_HASH);
 for ( ; start < MAX_KEY_HASH*2; start++ ) {
  int key=start%MAX_KEY_HASH;
  if ( room_index_hash[key] ) {
   int hashed=0;
   ROOM_INDEX_DATA *r;
   for ( r=room_index_hash[key]; r; r=r->next ) hashed++;
   int selected=number_range(0,hashed);
   r=room_index_hash[key];
   while ( r->next && (--selected) > 0 ) r=r->next;
   return r;
  }
 }
 return NULL;
