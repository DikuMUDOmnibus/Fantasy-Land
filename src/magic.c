...
void do_cast( CHAR_DATA *ch, char *argument )
{
    ...
    {
	sprintf(buf,"%s is a skill not a spell!\n\r",
...
//	gate_pet = FALSE;
    ...
    CHAR_DATA *pet;
    for ( pet=char_list; pet; pet=pet->next) if ( IS_NPC(pet) && pet->master ==ch && pet->in_room == ch->in_room )
    {
	act("$n steps through a gate and vanishes.",pet,NULL,NULL,TO_ROOM,POS_RESTING);
	send_to_char("You step through a gate and vanish.\n\r",pet);
	char_from_room(pet);
	char_to_room(pet,victim->in_room);
	act("$n has arrived through a gate.",pet,NULL,NULL,TO_ROOM,POS_RESTING);
	do_look(pet,"auto");
    }
    return TRUE;
}
