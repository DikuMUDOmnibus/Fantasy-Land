...
void do_mpassist( CHAR_DATA *ch, char *argument )
{
    ...
    multi_hit( ch, victim->fighting, TYPE_UNDEFINED, TRUE );
    return;
}
...
void do_mptransfer( CHAR_DATA *ch, char *argument )
{
    ...
    CHAR_DATA * pet;
    for ( pet=char_list; pet; pet=pet->next ) if ( pet->master ==victim && IS_NPC(pet) ) {
	char_from_room( pet );
    ...
    if ( IS_NPC( victim ) && HAS_TRIGGER_MOB( victim, TRIG_ENTRY ) )
        p_percent_trigger( victim, NULL, NULL, NULL, NULL, NULL, TRIG_ENTRY );
    if ( !IS_NPC( victim ) )
...
void do_mpdamage( CHAR_DATA *ch, char *argument )
{
	...
	for( victim = ch->in_room->people; victim; victim = victim_next )
	{
	    ...
	    if ( victim != ch )
	        ...
	        TYPE_UNDEFINED, DAM_OTHER, FALSE, FALSE, NULL );
	}
    }
    else
    	damage( victim, victim, 
...
void do_mpstate( CHAR_DATA *ch, char *argument )
{
    ...
    {
        bug( "MpState: invalid args from vnum %d.",
                IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
        return;
    }
    ...
    if ( !is_number( arg2 ) )
    {
        bug( "MpState: state number isn't a number from vnum %d.",
                IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
        return;
    }

    if ( !str_cmp(arg1, "+=") )
	ch->mprog_state += atoi( arg2);
    ...
    {
        bug( "MpState: invalid operator from vnum %d.",
                IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
        return;
    }
}
...
MEM_DATA *get_mem_data(CHAR_DATA *ch, CHAR_DATA *target)
{
    ...
    {
	if ( remember && target->id && remember->id == target->id )
	    return remember;
    }
...
void mob_remember(CHAR_DATA *ch, CHAR_DATA *target, int reaction)
{
    ...
    if ( reaction == MEM_AFRAID && IS_SET( remember->reaction, MEM_HOSTILE ) )
	REMOVE_BIT( remember->reaction, MEM_HOSTILE );
...
void do_opechoat( OBJ_DATA *obj, char *argument )
{
    ...
    act( argument, obj->carried_by?obj->carried_by:obj->in_room->people, obj, victim, TO_VICT, POS_RESTING );
}
...
void do_opecho( OBJ_DATA *obj, char *argument )
{
    ...
    act( argument, obj->carried_by?obj->carried_by:obj->in_room->people, NULL, NULL, TO_ROOM, POS_RESTING );
    act( argument, obj->carried_by?obj->carried_by:obj->in_room->people, NULL, NULL, TO_CHAR, POS_RESTING );
}
...
void do_rpasound( ROOM_INDEX_DATA *room, char *argument )
{
    ...
    for ( door = 0; door < 6; door++ )
    {
	  ...
	  &&   pexit->u1.to_room->people != NULL )
      	{
	    act( argument, pexit->u1.to_room->people, NULL, NULL, TO_ROOM, POS_RESTING );
	    act( argument, pexit->u1.to_room->people, NULL, NULL, TO_CHAR, POS_RESTING );
	}
    }
    return;
...
void do_rpechoaround( ROOM_INDEX_DATA *room, char *argument )
{
    ...
    act( argument, victim, NULL, victim, TO_NOTVICT, POS_RESTING );
}
...
void do_rpechoat( ROOM_INDEX_DATA *room, char *argument )
{
    ...
    act( argument, victim, NULL, NULL, TO_CHAR, POS_RESTING );
}
...
{    if ( argument[0] == '\0' )
    ...
    act( argument, room->people, NULL, NULL, TO_ROOM, POS_RESTING );
    act( argument, room->people, NULL, NULL, TO_CHAR, POS_RESTING );
}
...
void do_rpcast( ROOM_INDEX_DATA *room, char *argument )
{
    CHAR_DATA *vch;
    void *victim = NULL;
    char spell[ MAX_INPUT_LENGTH ],
	 target[ MAX_INPUT_LENGTH ],
	 levelchar[ MAX_INPUT_LENGTH ];
    int sn, level;

    argument = one_argument( argument, spell );
    argument = one_argument( argument, target );
               one_argument( argument, levelchar );

    if ( spell[0] == '\0' )
    {
	bug( "RpCast - Bad syntax from vnum %d.", 
		room->vnum );
	return;
    }
    ...
    if ( ( sn = skill_lookup( spell ) ) < 0 )
    {
	bug( "RpCast - No such spell from vnum %d.", 
		room->vnum );
	return;
    }
    ...
    if (levelchar[0] == '\0')
        level = 101;
    else if ( !is_number(levelchar) )
    {
	bug( "RpCast - Level is not a number - specify or leave blank for 101 from vnum %d.", 
		room->vnum );
	return;
    }
    else
	...
	case TAR_OBJ_CHAR_OFF:
	case TAR_OBJ_INV:
	    bug( "RpCast - Invalid spelltype from vnum %d.", 
		room->vnum );
	    return;
    }
    (*skill_table[sn].spell_fun)( sn, level, vch, victim,
	skill_table[sn].target);
    return;
}
