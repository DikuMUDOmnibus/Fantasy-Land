...
const char * fn_keyword[] =
{
    ...
    "base_class",	/* if base_class $n mage - is $n's class mage or wiz? */
    ...
    "state",            /* if state $i == 1        - mob state by Zarko */
 ...
 */
bool get_mob_vnum_room( CHAR_DATA *ch, OBJ_DATA *obj, ROOM_INDEX_DATA 
*room, sh_int vnum )
{
    CHAR_DATA *mob;
	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
	...
	case CHK_IMM:
	    return( lval_char != NULL && dam_type_lookup( buf ) != -1
		&& lval_char->damage_mod[dam_type_lookup( buf )] == 0 );
	case CHK_CARRIES:
	    if ( is_number( buf ) )
	...
	case CHK_HAS:
	   return( lval_char != NULL && has_item( lval_char, -1, flag_value( type_flags, buf ), FALSE ) );
	case CHK_USES:
	    return( lval_char != NULL && has_item( lval_char, -1, flag_value( type_flags, buf ), TRUE ) );
	case CHK_NAME:
            switch( code )
            {
                default :
	...
	case CHK_POS:
	    return( lval_char != NULL && lval_char->position == flag_value( position_flags, buf ) );
	case CHK_CLAN:
	...
	case CHK_BASE_CLASS:
	    return( lval_char != NULL && class_table[lval_char->class].sub_class == class_lookup(  buf ) );
	case CHK_OBJTYPE:
	    return( lval_obj != NULL && lval_obj->item_type == flag_value( type_flags, buf ) );
	default:;
    }
	...
	case CHK_GRPSIZE:
        ...
        case CHK_STATE:
            if( lval_char != NULL ) lval = lval_char->mprog_state; break;
	default:
            return FALSE;
    }
    return( num_eval( lval, oper, rval ) );
	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
	...
	case CHK_CLONES:
	    rval = count_people_room( NULL, obj, NULL, 3 ); break;
	case CHK_ORDER:
