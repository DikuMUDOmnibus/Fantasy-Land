...
int     find_rvnum_area         args( ( AREA_DATA *pArea ) );
...
int find_rvnum_area( AREA_DATA *pArea ) {
 int i=pArea->min_vnum;
 for ( ; i<=pArea->max_vnum; i++ ) {
...
bool hedit_immtext( CHAR_DATA *ch, char *argument )
{
    HELP_DATA *pHelp;
...
bool gredit_name( CHAR_DATA *ch, char *argument )
{
    ...
    int err=system( buf );
        if ( err < 0 ) log_string( "Error: gredit_name system call\n\r");
...
bool skedit_name( CHAR_DATA *ch, char *argument )
{
    ...
    int err=system( buf );
        if ( err < 0 ) log_string( "Error: skedit_name system call\n\r");
...
*/
    ...
    if ( !str_cmp( command, "to" ) || !str_cmp( command, "link" ) || is_number( command ) )
    {
	EXIT_DATA *pExit;

	if ( arg[0] == '\0' || !is_number( arg ) )
	{
	    send_to_char( "Syntax:  [direction] to [vnum]\n\r", ch );
	    send_to_char( "Syntax:  [direction] [vnum]\n\r", ch );
	    return FALSE;
	}
	...
	if ( !get_room_index( value ) )
	{
	    send_to_char( "REdit:  Cannot link to non-existant room.\n\r", ch );
            int nvnum=find_rvnum_area(ch->in_room->area);
            if ( nvnum > 0 ) {
             char buf2[MAX_STRING_LENGTH];
             sprintf( buf2, "dig %d", nvnum );
             return change_exit( ch, buf2, door );
            }
	    return FALSE;
	}
	...
	{
	    send_to_char( "Syntax: [direction] dig <optional vnum>\n\r", ch );
	    return FALSE;
	}
	...
	if ( !get_obj_index( value ) && (value != -1 || value != 0) )
	{
	    send_to_char( "REdit:  Item doesn't exist.\n\r", ch );
	    return FALSE;
	}

	if ( get_obj_index( atoi( argument ) )->item_type != ITEM_KEY && (value != 0 || value !=-1) )
	{
	...
	pRoom->exit[door]->key = value;
        ...
        if ( value == -1 )
        send_to_char( "Exit is now unwalkable (for windows) or has a keyless lock (when locked).\n\r", ch );
        else
...
OLC( class_edit_name )
{
    ...
    int err=system( buf );
        if ( err < 0 ) log_string( "Error: class_edit_name system call\n\r");
...
OLC( race_edit_name )
{
    ...
    int err=system( buf );
        if ( err < 0 ) log_string( "Error: race_edit_name system call\n\r");
...
OLC( clan_edit_name )
{
    ...
    int err=system( buf );
        if ( err < 0 ) log_string( "Error: clan_edit_name system call\n\r");
