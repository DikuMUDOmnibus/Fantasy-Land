...
void fwrite_obj         args( ( OBJ_DATA *obj, FILE *fp, int iNest, int storage, bool special ) );
...
void do_reload( CHAR_DATA *ch, char *argument )
{
	...
	sprintf( buf, "cp %s%s/%s %s%s/%s", BACKUP_DIR, initial( argument ),
	    ...
	    capitalize( argument ) );
	int err=system( buf );
        if ( err < 0 ) log_string( "Error: player backup system call\n\r");
...
void fwrite_char( CHAR_DATA *ch, FILE *fp, bool clan_save )
{
    ...
    if ( ch->pcdata != NULL )
    {
        if ( ch->pcdata->home != 0 ) fprintf( fp, "Home %d\n", ch->pcdata->home );
	fprintf( fp, "Pkpt %d\n",	ch->pcdata->pkpoints		);
	fprintf( fp, "TSex %d\n",	ch->pcdata->true_sex		);
	fprintf( fp, "LogO %ld\n",
...
void fwrite_pet( CHAR_DATA *ch, CHAR_DATA *pet, FILE *fp)
{
    ...
    if ( pet->carrying != NULL )
	    fwrite_obj( pet->carrying, fp, 0, -1, FALSE );
...
CHAR_DATA *fread_pet( CHAR_DATA *ch, FILE *fp )
{
    char *word;
    CHAR_DATA *pet;
    bool fMatch;
    ...
    for ( ; ; )
    {
		...
		pet->leader = ch;
		pet->master = ch;
//                pet->next=char_list;  // this happened already in create_mob
//                char_list=pet;
...
//	obj->wear_loc = WEAR_NONE;
	...
	case 'E':
		...
		{
		    bug( "Fread_obj: Unlinked vnum %d. (what the fuck does this mean)", // Locke
			obj->pIndexData->vnum );
                    ...
                    bool arena_load, bool reload )
{
    ...
    ch->shielded_by			= 0;
    ch->size				= 0;
    ch->prompt 				= str_dup("(%e){1<{@%hh %mm %vv{1> {1[{@%q{1]{x ");
// str_dup("{w(%e) <{M%h{w/{m%H{whp> <{B%M{w/{b%m{wm> <{G%v{w/{g%V{wmv> {R-=+>{C%X{R<+=-{X");
//str_dup("{x<{R%A{x/{r%H{xhp {B%B{x/{b%M{xm {G%Cmv {x<%X{Wxp{x>");
...
// writes the saving content room contents -Locke
...
void fwrite_rooms() {
 ...
 if ( fp != NULL ) {
    ...
    int i=0;
    for ( ; i<MAX_KEY_HASH; i++ ) for ( pRoomIndex  = room_index_hash[i];
          pRoomIndex != NULL;
          pRoomIndex  = pRoomIndex->next )
    if ( IS_SET(pRoomIndex->room_flags,ROOM_SAVE_CONTENTS) && pRoomIndex->contents ) {
     fwrite_obj( pRoomIndex->contents, fp, 0, -1, FALSE );
    }
  fclose(fp);
 }
}
