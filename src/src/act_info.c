...
void do_prompt(CHAR_DATA *ch, char *argument)
{
   ...
   if ( !strcmp (argument, "default" ) )
      strcpy (buf, "{W({w%e{W) {w%A{Dhp {w%B{Dmn {w%C{Dmv {g%X{Dtnl{w >{W>{x");
   else if ( !strcmp (argument, "all" ) )
      strcpy ( buf, "{W({w%e{W) {w%h{W/{w%H{Dhp {w%m{W/{w%M{Dmn {w%v{Dmv {y%X{Dxp tnl{w >{W>{x");
   else if( !strcmp( argument, "basic" ) )
      strcpy( buf, "{x<%Ahp %Bm %Cmv> [{1%X{x]%c");
   else
   {
...
void do_look( CHAR_DATA *ch, char *argument )
{
    ...
    {
        /* 'look' or 'look auto' */
        sprintf( buf, "{e%s{x", ch->in_room->name );
	...
	{
	default:
	    send_to_char( "That is not a container dumbass.\n\r", ch );
	    break;

	case ITEM_DRINK_CON:
	    if ( in_obj->value[1] <= 0 )
	    {
		send_to_char( "It is empty so stop sucking it.\n\r", ch );
		break;
	    }
...
void do_help( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char( "Help file not found, use the nohelp command to log it.\n\r", ch );
}
...
char * display_who_default( CHAR_DATA *ch, CHAR_DATA *wch, bool fAll )
{
    ...
    char arena[30], level[7], wizi[20], incog[20], ghost[20], quest[5];
	...
	sprintf( quest, "{R*{x" );
// do_who
    sprintf( buf, "{r ({x %s {r){x%s{x%s%s%s%s%s%s%s%s%s%s%s%s%s%s",
	string, quest,
        (clan_table[wch->clan].who_name != NULL) ? clan_table[wch->clan].who_name : "",
...
void do_who( CHAR_DATA *ch, char *argument )
{
    ...
    final	 = new_buf();
     ...
     add_buf(final,"                Currently Playing on {rA{Rs{yg{Ya{Wr{Yd{yi{Ra{rn {DN{wightmare{W:{x\n\r  ");
//     add_buf(final,"{D---------------{w--------------{W--------------{w--------------{D---------------{x\n\r");
    ...
    if ( mud_stat.exp_mod[0] != 1 || mud_stat.exp_mod[1] != 1 )
    {
       if ( mud_stat.exp_mod[1] == 1 )
           sprintf( buf, "{Y\n\rExperience Gain: %dX{x",
           mud_stat.exp_mod[0] );
       else
           sprintf( buf, "{Y\n\rExperience Gain: %d/%d{x",
           mud_stat.exp_mod[0], mud_stat.exp_mod[1] );
           add_buf( final, buf );
    }
    else
    {
           sprintf( buf, "{Y\n\rExperience Gain: NORMAL");
           add_buf( final, buf );
    }                                                                                            
	...
	sprintf(buf,"\n\r{wPlayers visible{d:  {1({Y%-3d{r){x  {wTotal Connections since reboot{d:  {r({Y%-3d{r){x\n\r",
	    count, mud_stat.connect_since_reboot );
	add_buf(final,buf);
       /*
