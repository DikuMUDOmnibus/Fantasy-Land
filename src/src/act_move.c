...
void do_open( CHAR_DATA *ch, char *argument )
{
	...
	act( "You open the $d.", ch, NULL, pexit->keyword, TO_CHAR, POS_RESTING );
...
void do_wake( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(victim) && !can_pk(ch,victim) )
    {
...
void do_heel( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->pet == NULL )
    {
	send_to_char( "You do not own a pet.\n\r", ch );
	return;
    }
    ...
    {
	act( "$N is right here!",
	    ch, NULL, ch->pet, TO_CHAR, POS_RESTING );
	return;
    }
	...
	ch->pet, NULL, NULL, TO_ROOM, POS_RESTING );
