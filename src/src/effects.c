...
void acid_effect( CHAR_DATA *ch, void *vo, int level, int dam, int target )
{
	...
	for (obj = room->contents; obj != NULL; obj = obj_next)
	{
	    ...
	    acid_effect(ch,obj,level,dam,TARGET_OBJ);
	}
	return;
    }
	...
	if (obj->item_type == ITEM_ARMOR)  /* etch it */
	{
	    ...
	    {
                for ( i = 0; i < 4; i++ )
                    obj->carried_by->armor[i] += mod;
		obj->carried_by->hitroll -= mod;
		obj->carried_by->damroll -= mod;
	    }
	...
	if (obj->contains)  /* dump contents */
	{
		...
		acid_effect(ch,t_obj,level/2,dam/2,TARGET_OBJ);
	    }
 	}
...
void cold_effect( CHAR_DATA *ch, void *vo, int level, int dam, int target )
{
    if (target == TARGET_ROOM) /* nail objects on the floor */
    {
        ROOM_INDEX_DATA *room = (ROOM_INDEX_DATA *) vo;
        ...
        {
            obj_next = obj->next_content;
            cold_effect(ch,obj,level,dam,TARGET_OBJ);
        }
        return;
    }
	...
	/* chill touch effect */
	if (!saves_spell(level/4 + dam / 20, ch, victim, DAM_COLD))
	{
	    AFFECT_DATA af;
...
void fire_effect( CHAR_DATA *ch, void *vo, int level, int dam, int target )
{
    if (target == TARGET_ROOM)  /* nail objects on the floor */
    {
	ROOM_INDEX_DATA *room = (ROOM_INDEX_DATA *) vo;
	...
	{
	    obj_next = obj->next_content;
	    fire_effect(ch,obj,level,dam,TARGET_OBJ);
	}
	return;
    }
	...
	&&  !saves_spell(level / 4 + dam / 20, ch, victim,DAM_FIRE))
	{
            AFFECT_DATA af;
	...
	if (obj->carried_by != NULL)
		...
		fire_effect(ch,t_obj,level/2,dam/2,TARGET_OBJ);
            }
        }
...
void poison_effect( CHAR_DATA *ch, void *vo,int level, int dam, int target )
{
    if (target == TARGET_ROOM)  /* nail objects on the floor */
    {
        ROOM_INDEX_DATA *room = (ROOM_INDEX_DATA *) vo;
        ...
        {
            obj_next = obj->next_content;
            poison_effect(ch,obj,level,dam,TARGET_OBJ);
        }
        return;
    }
	...
	/* chance of poisoning */
        if (!saves_spell(level / 4 + dam / 20,ch,victim,DAM_POISON))
        {
	    AFFECT_DATA af;
...
void shock_effect( CHAR_DATA *ch, void *vo, int level, int dam, int target )
{
	...
	{
	    obj_next = obj->next_content;
	    shock_effect(ch,obj,level,dam,TARGET_OBJ);
	}
	return;
    }
	...
	if (!saves_spell(level/4 + dam/20,ch,victim,DAM_LIGHTNING))
	{
