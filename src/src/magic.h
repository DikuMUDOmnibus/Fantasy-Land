/***************************************************************************
 *  Original Diku Mud copyright (C) 1990, 1991 by Sebastian Hammer,	   *
 *  Michael Seifert, Hans Henrik St{rfeldt, Tom Madsen, and Katja Nyboe.   *
 *									   *
 *  Merc Diku Mud improvments copyright (C) 1992, 1993 by Michael	   *
 *  Chastain, Michael Quan, and Mitchell Tse.				   *
 *									   *
 *  In order to use any part of this Merc Diku Mud, you must comply with   *
 *  both the original Diku license in 'license.doc' as well the Merc	   *
 *  license in 'license.txt'.  In particular, you may not remove either of *
 *  these copyright notices.						   *
 *									   *
 *  Much time and thought has gone into this software and you are	   *
 *  benefitting.  We hope that you share your changes too.  What goes	   *
 *  around, comes around.						   *
 ***************************************************************************/

/***************************************************************************
*	ROM 2.4 is copyright 1993-1995 Russ Taylor			   *
*	ROM has been brought to you by the ROM consortium		   *
*	    Russ Taylor (rtaylor@pacinfo.com)				   *
*	    Gabrielle Taylor (gtaylor@pacinfo.com)			   *
*	    Brian Moore (rom@rom.efn.org)				   *
*	By using this code, you have agreed to follow the terms of the	   *
*	ROM license, in the file Rom24/doc/rom.license			   *
***************************************************************************/

/***************************************************************************
*       ROT 1.4 is copyright 1996-1997 by Russ Walsh                       *
*       By using this code, you have agreed to follow the terms of the     *
*       ROT license, in the file doc/rot.license                           *
***************************************************************************/

DECLARE_SPELL_FUN( spell_absorb_magic		);
DECLARE_SPELL_FUN( spell_acid_blast		);
DECLARE_SPELL_FUN( spell_acid_breath		);
DECLARE_SPELL_FUN( spell_acid_storm		);
DECLARE_SPELL_FUN( spell_acidshield		);
DECLARE_SPELL_FUN( spell_angelfire		);
DECLARE_SPELL_FUN( spell_animate		);
DECLARE_SPELL_FUN( spell_armor			);
DECLARE_SPELL_FUN( spell_awen			);
DECLARE_SPELL_FUN( spell_backdraft		);
DECLARE_SPELL_FUN( spell_barkskin		);
DECLARE_SPELL_FUN( spell_bless			);
DECLARE_SPELL_FUN( spell_blindness		);
DECLARE_SPELL_FUN( spell_blink			);
DECLARE_SPELL_FUN( spell_blizzard		);
DECLARE_SPELL_FUN( spell_bloodbath		);
DECLARE_SPELL_FUN( spell_boiling_blood		);
DECLARE_SPELL_FUN( spell_burning_hands		);
DECLARE_SPELL_FUN( spell_call_lightning		);
DECLARE_SPELL_FUN( spell_calm			);
DECLARE_SPELL_FUN( spell_cancellation		);
DECLARE_SPELL_FUN( spell_cause_critical		);
DECLARE_SPELL_FUN( spell_cause_light		);
DECLARE_SPELL_FUN( spell_cause_serious		);
DECLARE_SPELL_FUN( spell_change_sex		);
DECLARE_SPELL_FUN( spell_chain_lightning	);
DECLARE_SPELL_FUN( spell_channel		);
DECLARE_SPELL_FUN( spell_charm_person		);
DECLARE_SPELL_FUN( spell_chill_touch		);
DECLARE_SPELL_FUN( spell_colour_spray		);
DECLARE_SPELL_FUN( spell_conjure		);
DECLARE_SPELL_FUN( spell_constitution		);
DECLARE_SPELL_FUN( spell_continual_light	);
DECLARE_SPELL_FUN( spell_control_weather	);
DECLARE_SPELL_FUN( spell_corrupt_potion		);
DECLARE_SPELL_FUN( spell_create_food		);
DECLARE_SPELL_FUN( spell_create_raft		);
DECLARE_SPELL_FUN( spell_create_rose		);
DECLARE_SPELL_FUN( spell_create_spring		);
DECLARE_SPELL_FUN( spell_create_water		);
DECLARE_SPELL_FUN( spell_cure_blindness		);
DECLARE_SPELL_FUN( spell_cure_critical		);
DECLARE_SPELL_FUN( spell_cure_disease		);
DECLARE_SPELL_FUN( spell_cure_light		);
DECLARE_SPELL_FUN( spell_cure_poison		);
DECLARE_SPELL_FUN( spell_cure_serious		);
DECLARE_SPELL_FUN( spell_cure_weaken		);
DECLARE_SPELL_FUN( spell_curse			);
DECLARE_SPELL_FUN( spell_darkvision		);
DECLARE_SPELL_FUN( spell_decrement		);
DECLARE_SPELL_FUN( spell_demonfire		);
DECLARE_SPELL_FUN( spell_detect_evil		);
DECLARE_SPELL_FUN( spell_detect_good		);
DECLARE_SPELL_FUN( spell_detect_hidden		);
DECLARE_SPELL_FUN( spell_detect_invis		);
DECLARE_SPELL_FUN( spell_detect_magic		);
DECLARE_SPELL_FUN( spell_detect_neutral		);
DECLARE_SPELL_FUN( spell_detect_poison		);
DECLARE_SPELL_FUN( spell_detect_terrain		);
DECLARE_SPELL_FUN( spell_deviant_point		);
DECLARE_SPELL_FUN( spell_devotion		);
DECLARE_SPELL_FUN( spell_diminish		);
DECLARE_SPELL_FUN( spell_dispel_evil		);
DECLARE_SPELL_FUN( spell_dispel_good		);
DECLARE_SPELL_FUN( spell_dispel_magic		);
DECLARE_SPELL_FUN( spell_dispel_neutral		);
DECLARE_SPELL_FUN( spell_divine_aura		);
DECLARE_SPELL_FUN( spell_divine_blessing	);
DECLARE_SPELL_FUN( spell_divine_heal		);
DECLARE_SPELL_FUN( spell_divine_truth		);
DECLARE_SPELL_FUN( spell_divinity		);
DECLARE_SPELL_FUN( spell_downpour		);
DECLARE_SPELL_FUN( spell_earthquake		);
DECLARE_SPELL_FUN( spell_electrical_storm	);
DECLARE_SPELL_FUN( spell_electrify		);
DECLARE_SPELL_FUN( spell_embalm			);
DECLARE_SPELL_FUN( spell_empower_potion		);
DECLARE_SPELL_FUN( spell_empower_scroll		);
DECLARE_SPELL_FUN( spell_enchant_armor		);
DECLARE_SPELL_FUN( spell_enchant_weapon		);
DECLARE_SPELL_FUN( spell_energy_curse		);
DECLARE_SPELL_FUN( spell_energy_drain		);
DECLARE_SPELL_FUN( spell_faerie_fire		);
DECLARE_SPELL_FUN( spell_faerie_fog		);
DECLARE_SPELL_FUN( spell_faith                  );
DECLARE_SPELL_FUN( spell_farsight		);
DECLARE_SPELL_FUN( spell_feeble_mind		);
DECLARE_SPELL_FUN( spell_fireball		);
DECLARE_SPELL_FUN( spell_fire_breath		);
DECLARE_SPELL_FUN( spell_fireproof		);
DECLARE_SPELL_FUN( spell_fireshield		);
DECLARE_SPELL_FUN( spell_fire_storm		);
DECLARE_SPELL_FUN( spell_flamestrike		);
DECLARE_SPELL_FUN( spell_flash			);
DECLARE_SPELL_FUN( spell_floating_disc		);
DECLARE_SPELL_FUN( spell_fly			);
DECLARE_SPELL_FUN( spell_frenzy			);
DECLARE_SPELL_FUN( spell_frost_breath		);
DECLARE_SPELL_FUN( spell_gas_breath		);
DECLARE_SPELL_FUN( spell_gate			);
DECLARE_SPELL_FUN( spell_general_purpose	);
DECLARE_SPELL_FUN( spell_giant_strength		);
DECLARE_SPELL_FUN( spell_glacial_aura		);
DECLARE_SPELL_FUN( spell_glacier		);
DECLARE_SPELL_FUN( spell_golden_armor		);
DECLARE_SPELL_FUN( spell_groupheal		);
DECLARE_SPELL_FUN( spell_growth			);
DECLARE_SPELL_FUN( spell_hail_storm		);
DECLARE_SPELL_FUN( spell_harm			);
DECLARE_SPELL_FUN( spell_haste			);
DECLARE_SPELL_FUN( spell_heal			);
DECLARE_SPELL_FUN( spell_heat_metal		);
DECLARE_SPELL_FUN( spell_heresy			);
DECLARE_SPELL_FUN( spell_high_explosive		);
DECLARE_SPELL_FUN( spell_holy_word		);
DECLARE_SPELL_FUN( spell_hurricane		);
DECLARE_SPELL_FUN( spell_iceshield		);
DECLARE_SPELL_FUN( spell_icicle			);
DECLARE_SPELL_FUN( spell_identify		);
DECLARE_SPELL_FUN( spell_incinerate		);
DECLARE_SPELL_FUN( spell_infernal_offering	);
DECLARE_SPELL_FUN( spell_infravision		);
DECLARE_SPELL_FUN( spell_intellect		);
DECLARE_SPELL_FUN( spell_invis			);
DECLARE_SPELL_FUN( spell_iron_skin		);
DECLARE_SPELL_FUN( spell_kailfli		);
DECLARE_SPELL_FUN( spell_know_alignment		);
DECLARE_SPELL_FUN( spell_leech			);
DECLARE_SPELL_FUN( spell_life_curse		);
DECLARE_SPELL_FUN( spell_lightning_bolt		);
DECLARE_SPELL_FUN( spell_lightning_breath	);
DECLARE_SPELL_FUN( spell_locate_object		);
DECLARE_SPELL_FUN( spell_locust_swarm		);
DECLARE_SPELL_FUN( spell_luminrati		);
DECLARE_SPELL_FUN( spell_magic_missile		);
DECLARE_SPELL_FUN( spell_mana_shield		);
DECLARE_SPELL_FUN( spell_mana_tap		);
DECLARE_SPELL_FUN( spell_mass_healing		);
DECLARE_SPELL_FUN( spell_mass_invis		);
DECLARE_SPELL_FUN( spell_memory_lapse		);
DECLARE_SPELL_FUN( spell_meteor_swarm		);
DECLARE_SPELL_FUN( spell_mindblast		);
DECLARE_SPELL_FUN( spell_negate_alignment	);
DECLARE_SPELL_FUN( spell_nexus			);
DECLARE_SPELL_FUN( spell_nightmare		);
DECLARE_SPELL_FUN( spell_null			);
DECLARE_SPELL_FUN( spell_pass_door		);
DECLARE_SPELL_FUN( spell_plague			);
DECLARE_SPELL_FUN( spell_poison			);
DECLARE_SPELL_FUN( spell_portal			);
DECLARE_SPELL_FUN( spell_power_of_gods		);
DECLARE_SPELL_FUN( spell_preserve		);
DECLARE_SPELL_FUN( spell_project_force		);
DECLARE_SPELL_FUN( spell_protection_evil	);
DECLARE_SPELL_FUN( spell_protection_good	);
DECLARE_SPELL_FUN( spell_protection_neutral	);
DECLARE_SPELL_FUN( spell_protection_voodoo	);
DECLARE_SPELL_FUN( spell_psi_barrier		);
DECLARE_SPELL_FUN( spell_psi_twister		);
DECLARE_SPELL_FUN( spell_ray_of_deceit		);
DECLARE_SPELL_FUN( spell_ray_of_truth		);
DECLARE_SPELL_FUN( spell_recharge		);
DECLARE_SPELL_FUN( spell_refresh		);
DECLARE_SPELL_FUN( spell_regeneration		);
DECLARE_SPELL_FUN( spell_remove_curse		);
DECLARE_SPELL_FUN( spell_restore_health		);
DECLARE_SPELL_FUN( spell_restore_magic		);
DECLARE_SPELL_FUN( spell_restore_mana		);
DECLARE_SPELL_FUN( spell_restore_movement	);
DECLARE_SPELL_FUN( spell_resurrect		);
DECLARE_SPELL_FUN( spell_rockshield		);
DECLARE_SPELL_FUN( spell_sanctuary		);
DECLARE_SPELL_FUN( spell_shield			);
DECLARE_SPELL_FUN( spell_shocking_grasp		);
DECLARE_SPELL_FUN( spell_shockshield		);
DECLARE_SPELL_FUN( spell_shrapnelshield		);
DECLARE_SPELL_FUN( spell_shrink			);
DECLARE_SPELL_FUN( spell_silence		);
DECLARE_SPELL_FUN( spell_sleep			);
DECLARE_SPELL_FUN( spell_slow			);
DECLARE_SPELL_FUN( spell_smite_health		);
DECLARE_SPELL_FUN( spell_smite_magic		);
DECLARE_SPELL_FUN( spell_smite_movement		);
DECLARE_SPELL_FUN( spell_snow_storm		);
DECLARE_SPELL_FUN( spell_soul_blade		);
DECLARE_SPELL_FUN( spell_stamina_curse		);
DECLARE_SPELL_FUN( spell_steel_skin		);
DECLARE_SPELL_FUN( spell_stone_skin		);
DECLARE_SPELL_FUN( spell_summon			);
DECLARE_SPELL_FUN( spell_swarm			);
DECLARE_SPELL_FUN( spell_teleport		);
DECLARE_SPELL_FUN( spell_thornshield		);
DECLARE_SPELL_FUN( spell_thunderbolt		);
DECLARE_SPELL_FUN( spell_transfix		);
DECLARE_SPELL_FUN( spell_transport		);
DECLARE_SPELL_FUN( spell_vampiricshield		);
DECLARE_SPELL_FUN( spell_ventriloquate		);
DECLARE_SPELL_FUN( spell_vigorize		);
DECLARE_SPELL_FUN( spell_voodoo			);
DECLARE_SPELL_FUN( spell_watershield		);
DECLARE_SPELL_FUN( spell_weaken			);
DECLARE_SPELL_FUN( spell_wisdom			);
DECLARE_SPELL_FUN( spell_wizard_eye		);
DECLARE_SPELL_FUN( spell_word_of_recall		);
